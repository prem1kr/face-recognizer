...
# Function to generate an Excel file with the recognition log data
def generate_excel_file():
    log_entries = log_text.get("1.0", tk.END).strip().split("\n")
    data = [entry.split(": ") for entry in log_entries if entry]
    
    # Sort the data by timestamp (the last element of each entry)
    ...
    messagebox.showinfo("Excel File", f"Excel file created at {data}")
    return data
...
def on_data_icon_click(event):
    ...
    if whatsapp_number:
        ...
        # Prompt user for Google Drive link (this can be used to store or track the file)
        google_drive_link = simpledialog.askstring("Google Drive Link", "Enter the Google Drive link (if applicable):")

        # Prepare the message to send
        if google_drive_link:
            message = f"Here is the recognition log report. You can download it [here]({google_drive_link})."
        else:
            message = "Here is the recognition log report. No Google Drive link provided."

        send_whatsapp(whatsapp_number, message)  # Send the message on WhatsApp
    else:
        messagebox.showerror("Error", "WhatsApp number is required to send the file.")

# Modify the WhatsApp sending function to handle text messages
def send_whatsapp(whatsapp_number, message):
    try:
        # Send a message with the link
        pwk.sendwhatmsg(whatsapp_number, message, datetime.now().hour, datetime.now().minute + 2)  # Send the message after 2 minutes
        messagebox.showinfo("WhatsApp", "Message sent successfully on WhatsApp!")
    except Exception as e:
        messagebox.showerror("Error", f"Failed to send the message on WhatsApp: {e}")
